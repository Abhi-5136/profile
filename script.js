...
function initCertAccordion() {
    ...
    accordionHeaders.forEach(header => {
        header.addEventListener('click', function() {
            ...
            document.querySelectorAll('.cert-accordion-item').forEach(otherItem => {
                otherItem.classList.remove('active');
                if (otherItem.querySelector('.cert-accordion-body')) {
                    otherItem.querySelector('.cert-accordion-body').style.maxHeight = null;
                }
            });
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initCertAccordion);
} else {
    initCertAccordion();
}
...
function initProjectsSlider() {
    ...
    if (projectsGrid && slideLeftBtn && slideRightBtn) {
        // Scroll distance roughly equals width of one card + gap
        const scrollDistance = 340; 
        
        slideLeftBtn.addEventListener('click', () => {
            projectsGrid.scrollBy({ left: -scrollDistance, behavior: 'smooth' });
        });
